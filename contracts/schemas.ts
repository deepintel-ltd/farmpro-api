...
export const CommoditySchema = z.object({
  ...
  category: z.enum(['grain', 'vegetable', 'fruit', 'livestock'], { errorMap: () => ({ message: 'Invalid commodity category' }) }),
  variety: z.string().optional(),
  qualityGrade: z.enum(['premium', 'standard', 'utility'], { errorMap: () => ({ message: 'Invalid quality grade' }) }),
  quantity: z.number().positive('Quantity must be positive'),
  unit: z.enum(['bushel', 'pound', 'ton', 'head'], { errorMap: () => ({ message: 'Invalid unit type' }) }),
...
export const UserSchema = z.object({
  ...
  role: z.enum(['farmer', 'buyer', 'admin'], { errorMap: () => ({ message: 'Invalid user role' }) }),
...
export const InventorySchema = z.object({
  ...
  harvestId: CuidQueryParam('id').optional(),
  quantity: z.number().positive('Quantity must be positive'),
...
export const InventoryTransferRequestSchema = z.object({
  fromInventoryId: CuidQueryParam('id'),
  toLocation: z.object({
    farmId: CuidQueryParam('id'),
    facility: z.string(),
    section: z.string().optional()
  }),
  quantity: z.number().positive(),
...
export const OrganizationSchema = z.object({
  ...
  type: z.enum(['individual', 'cooperative', 'corporation', 'nonprofit'], { errorMap: () => ({ message: 'Invalid organization type' }) }),
  isActive: z.boolean().default(true),
...
export const OrganizationActivityQuerySchema = z.object({
  ...
  userId: CuidQueryParam('id').optional()
});
...
export const OrganizationExportRequestSchema = z.object({
  ...
  format: z.enum(['json', 'csv', 'excel'], { errorMap: () => ({ message: 'Invalid export format' }) }),
  dateRange: z.object({
...
export const OrganizationBackupRequestSchema = z.object({
  ...
  retention: z.enum(['30d', '90d', '1y'], { errorMap: () => ({ message: 'Invalid retention period' }) })
});
...
export const OrganizationBillingSchema = z.object({
  planId: z.string(),
  billingCycle: z.enum(['monthly', 'yearly'], { errorMap: () => ({ message: 'Invalid billing cycle' }) }),
...
export const OrganizationTeamStatsSchema = z.object({
  period: z.string(),
  roleId: CuidQueryParam('id').optional(),
  farmId: CuidQueryParam('id').optional(),
