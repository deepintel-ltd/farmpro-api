...
/**
 * Simplified TestContext tests that focus on database functionality
 * without the full NestJS application setup
 */
describe('TestContext - Database Only', () => {
  let testContext: TestContext;

  beforeAll(async () => {
    testContext = new TestContext();
    // Only start the database, skip NestJS app setup for now
    await testContext['_dbManager'].startContainer();
    testContext['_prisma'] = testContext['_dbManager'].getPrismaClient();
  }, 60000);

  afterAll(async () => {
    if (testContext['_dbManager']) {
      await testContext['_dbManager'].stopContainer();
    }
  }, 30000);
  ...
  describe('Factory Methods', () => {
    ...
    // Note: Inventory tests skipped because inventory table doesn't exist in current schema
  });
